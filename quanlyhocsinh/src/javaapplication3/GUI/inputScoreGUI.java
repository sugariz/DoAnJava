...
import javaapplication3.BUS.giaoVienBoMonBUS;
...
public class inputScoreGUI {
    ...
    private void build(JPanel myPanel) {
        ...
        this.configPanel(panelFilter, Color.WHITE, new Dimension(950, 50));
        ...
        this.buildPanelFileter();
        
        myPanel.add(this.panelOption);
        myPanel.add(this.panelChangeData);
        myPanel.add(this.panelFilter);
        myPanel.add(this.scrollPanelScore);

    }
    
    private void buildPanelFileter() {
        JLabel labelLoc = new JLabel("Nhập tên học sinh");
        JTextField filter = new JTextField();
        JButton buttonFilter = new JButton("Lọc");
        this.configButton(buttonFilter, new Dimension(100, 30));
        this.configInput(filter, new Dimension(100, 30), false, false);
        panelFilter.add(labelLoc);
        ...
        buttonFilter.addActionListener(new ActionListener() { 
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                 ArrayList<Object[]> dataRow = createData1(thongTinDiem,filter.getText());
    ...
    private ArrayList<Object[]> createData1(ArrayList<infoClassDTO> thongTinDiem, String tenHS) {
        ArrayList<Object[]> dataBody = new ArrayList<Object[]>();
        for(int i = 0; i < thongTinDiem.size(); i++) {
            
            int index = 0;
            infoClassDTO ttd = thongTinDiem.get(i);
            if(String.format("%s %s",ttd.getSurname(), ttd.getName()).equals(tenHS)) {
                Object[] rowData = new Object[3 + 2 + 2 + 2 + 1];
                rowData[index] = i + 1;
                index += 1;
                // filter\
...
//    private ArrayList<Object[]> createData2(ArrayList<infoClassDTO> thongTinDiem, int score1, int score2) {
//        ArrayList<Object[]> dataBody = new ArrayList<Object[]>();
//        for(int i = 0; i < thongTinDiem.size(); i++) {
//            
//            int index = 0;
...
//    }
    ...
    private void buildPanelChangeData() {
        ...
        JButton exportCSV = new JButton("Xuất bảng điểm");
        JButton exportPDF = new JButton("Xuất file PDF");
        JButton printScore = new JButton("In");
        ...
        this.action(exportCSV, maGv, jComboBoxMonHoc.getSelectedItem().toString(), headers, dataBody);
        this.actionPDF(exportPDF, maGv, jComboBoxMonHoc.getSelectedItem().toString(), headers, dataBody);
        this.actionPrint(printScore, maGv, jComboBoxMonHoc.getSelectedItem().toString(), headers, dataBody);
        ...
        this.panelChangeData.add(exportCSV);
        this.panelChangeData.add(exportPDF);
        this.panelChangeData.add(printScore);
    ...
    private void action(JButton myButton, String maLop, String monHoc, String[] header, ArrayList<Object[]> data) {
        ActionListener btnActionListener = new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String fileName = maLop + monHoc + " - score.csv";
                new writeCSV(fileName).writeCSVScoreTeacher(header, data);
            }
        };
        myButton.addActionListener(btnActionListener);
    }
    
    private void actionPDF(JButton myButton, String maLop, String monHoc, String[] header, ArrayList<Object[]> data) {
        ActionListener btnActionListener = new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String fileName = maLop + monHoc + " - score.pdf";
                new writePDF(fileName).writePDFTeacher(header, data);
            }
        };
        myButton.addActionListener(btnActionListener);
    }
    
    private void actionPrint(JButton myButton, String maLop, String monHoc, String[] header, ArrayList<Object[]> data) {
        ActionListener btnActionListener = new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String fileName = maLop + monHoc + " - score.pdf";
